...
function validateY(recursion){
    ...
    if ( text==="" || isNaN(number_text) || !isFinite(number_text) || number_text < -3 || number_text > 3){
        ...
        y_message_place.innerHTML = "<p>Incorrect value<br>Enter from -3 to 3</p>";
    } else {
...
function getCursorPosition(canvas, event) {
    ...
    document.getElementById("thisIsForm:sliderX").value = x.toFixed(2);
    document.getElementById("thisIsForm:y_value").value = y.toFixed(2);
    submit_button.click();
    document.getElementById("thisIsForm:sliderX").value = -5;
...
function redraw_canvas(r_value){
    ...
    // 2 quarter
    context.beginPath();
    context.arc(0,0, Math.floor(r_value/2 * canvas_scale), Math.PI/2, Math.PI);
    context.moveTo(0, 0);
...
function mybigupdater(){
    const r = document.getElementById("thisIsForm:spinnerR_input").value.replace(',', '.');
    console.log(r);
    redraw_canvas(r);
}
